exports.run = async (client, msg, args) => {
   ...
   description: ('<a:739589662120738836:739589662120738836> Ya geçerli birini etiketle ya da sana sövmemi istiyosan kendini etiketle.')
  }});
    if(member.id === "662349193180086312")return msg.channel.send({embed: {
   color: Math.floor(Math.random() * (0xFFFFFF + 1)),
   description: ('<a:739589662120738836:739589662120738836> Sen Kimsin Sahibime Sövüyon Amına Kodumun Jedayı.')
  }})
    if(member.id === "474561265545773057")return msg.channel.send({embed: {
   color: Math.floor(Math.random() * (0xFFFFFF + 1)),
   description: (`<a:737724927591055411:737724927591055411> Hoop! <@474561265545773057>'yı  etiketlemişsin yarram!`)
  }})
    if(member.id === "662349193180086312")return msg.channel.send({embed: {
   color: Math.floor(Math.random() * (0xFFFFFF + 1)),
   description: (`<a:739589662120738836:739589662120738836> Hoop! Orda durucan Yapımcım <@662349193180086312>'e sövemezsin amını sikerim.`)
  }})
    if(member.id === client.user.id){
      msg.channel.send({embed: {
   color: Math.floor(Math.random() * (0xFFFFFF + 1)),
   description: (`<a:739589662120738836:739589662120738836> Beni mi kandırcan orospu çocuğu ?`)
  }})
    }
    else{
    msg.channel.send({embed: {
